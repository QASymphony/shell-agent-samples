@echo off
rem This is a utility to parse NUnit XML files into JSON formats required by qTest APIs.
...
pushd %LOCALREPO% & call msbuild %LOCALREPO%.csproj & popd

echo RUNNING TESTS

for /f tokens^=*^ delims^=^ eol^= %%i in ('call python getscheduledtests.py') do set TEST=%%i
for /f tokens^=*^ delims^=^ eol^= %%i in ('call python runscheduledtests.py "%LOCALREPO%"') do set DLLFILE=%%i
IF "%DLLFILE%" == "None" (
    ...
    pushd %LOCALREPO%\bin\"Debug" & call nunit3-console %DLLFILE% & popd
